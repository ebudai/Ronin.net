...
CANDIDATES = ['otherwise', 'to', 'of', 'in', 'by', 'with', 'on', 'where',
              'times', 'places', 'then', 'each', 'until', 'against', 'else',
              'unless', 'from', 'for', 'and', 'or', 'as', 'at']
...
def main(roots):
    ...
    print(f'  «to» ranks '
          f'{next(i for i, (w, _) in enumerate(rank) if w == "to") + 1}')
    print(f'  «of» ranks '
          f'{next(i for i, (w, _) in enumerate(rank) if w == "of") + 1}')
    print()
