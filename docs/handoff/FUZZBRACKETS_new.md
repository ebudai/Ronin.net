...
Known gap: `fuzz_pairs.py` has no `strict` policy, so there is no pairs baseline
of the kind `fuzz_leading.py` provides. Worth adding before the pair figures are
used to argue anything comparative.
