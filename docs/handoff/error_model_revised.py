...
class GuardedGraph(Graph):
    ...
    def __init__(self):
        ...
        self.suppressed = 0           # depth of `handling` frames

    def handling(self, thunk):
        """A NON-ARMING read. `otherwise` is the one thing that catches, so it
        must be able to see an error without adoption firing behind its back.

        Without this, `otherwise` is defeated completely: the read records the
        error, `otherwise` returns the fallback, and adoption then overrides
        the fallback with the very error it just handled."""
        self.suppressed += 1
        try:
            return thunk()
        finally:
    ...
    def read(self, name):
        ...
        if isinstance(value, RoninError) and self.seen_error and not self.suppressed:
            self.seen_error[-1] = value       # arm adoption
        return value
...
h('5. adoption must NOT fire behind `otherwise`')
g = GuardedGraph()
g.var('parsed', NOTHING)
g.let('naive', lambda e: otherwise(e.read('parsed'), 0))
g.let('correct', lambda e: otherwise(e.handling(lambda: e.read('parsed')), 0))
g.write('parsed', RoninError('bad input')); g.step()
naive, correct = g.read('naive'), g.read('correct')
results.append(ok('a plain read DEFEATS otherwise (adoption overrides it)',
                  isinstance(naive, RoninError)))
results.append(ok('reading through `handling` lets otherwise work', correct == 0))
print(f'      naive   -> {naive!r}   <- the fallback was overridden')
print(f'      correct -> {correct!r}')
print("""
      Found by the programmer while porting: this file never had an
      `otherwise` scenario, so the hole could not surface here. The triage
      predicted the requirement in prose and the executable version did not
      encode it -- the same gap diagnosed in SCOPING.md, one document later.""")

h('6. what the doc should say')
